...
run_reverse_proxy_integration() {
  ...
  log_info "Integration VM lifecycle for member ${LAB_HOST} (slice=reverse_proxy)"
  "${ROOT}/scripts/lab/vm-destroy.sh" "${LAB_HOST}" || true
  "${ROOT}/scripts/lab/vm-create.sh" "${LAB_HOST}"
  "${ROOT}/scripts/lab/wait-ssh.sh" "${LAB_HOST}"
  ...
  log_info "Converging bastion on ${LAB_HOST} (UFW baseline)"
  run_ansible_playbook "${ROOT}" playbooks/bastion.yml --limit "${LAB_HOST}"

  log_info "Converging reverse proxy on ${LAB_HOST} (first run)"
  run_ansible_playbook "${ROOT}" playbooks/reverse-proxy.yml --limit "${LAB_HOST}"

  log_info "Converging reverse proxy on ${LAB_HOST} (idempotency check)"
  assert_ansible_playbook_idempotent "${ROOT}" playbooks/reverse-proxy.yml --limit "${LAB_HOST}"

  log_info "Running reverse proxy convergence assertions"
  run_ansible_playbook "${ROOT}" tests/integration/test_reverse_proxy_converged.yml --limit "${LAB_HOST}"
