...
_PARSE_LIB=""
for _candidate in \
  "${_HOOK_DIR}/lib/dhcp-ddns-parse.sh" \
  "${_HOOK_DIR}/dhcp-ddns-parse.sh" \
  "/usr/local/lib/dhcp-ddns-parse.sh"; do
  if [[ -f "${_candidate}" ]]; then
    _PARSE_LIB="${_candidate}"
    break
  fi
done
if [[ -z "${_PARSE_LIB}" ]]; then
  die "missing dhcp-ddns-parse.sh (install lib next to hook or under lib/)"
fi
