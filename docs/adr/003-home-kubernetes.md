...
4. **Network — VLAN 9** (`192.168.9.0/24`) as a **routed** Kubernetes fabric (not L2-only
   ...
   - Pod/service CIDRs use non-`192.168.0.0/16` ranges (kubeadm defaults `10.244.0.0/16`,
     `10.96.0.0/12`) to avoid overlap with home VLANs.
...
5. **Edge — hybrid model** (revises ADR 001’s “no K8s ingress” for *public* edge only):
   ...
   - In-cluster **ingress-nginx** receives HTTP from proxy01.
   - **MetalLB L2** advertises a stable VIP on VLAN 9 for the Ingress `LoadBalancer` Service.

6. **CNI:** **Calico** (NetworkPolicy from early labs).
...
| MetalLB pool | `192.168.9.200`–`192.168.9.210` | L2 VIP for ingress-nginx |

Document static DNS A records for CP, workers, and the ingress VIP. Reserve pool addresses
...
| Flannel CNI | Fewer NetworkPolicy knobs; Calico matches enterprise practice |
