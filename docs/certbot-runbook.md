...
| LE NXDOMAIN for a *new* SAN while auth hook succeeded | Confirm the **public hostname** exists first (`dig +short <name> CNAME`). `uptime.2123studios.com` had no record at all until a Dreamhost CNAME → `bastion` was added; retries alone cannot validate a missing name |
