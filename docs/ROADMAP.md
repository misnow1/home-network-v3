...
| 2 | 23 | Restic scheduling + offsite copy | **in progress** | Nightly kif repo + AD live (PR #53). Restore drill still pending. Live USB go-bag in progress (#55). `/media/projects` archive is #54. — [backup-runbook.md](backup-runbook.md) |
...
| 23+ | Restic air-gap offsite | Slice 23, PR #53 | Portable *live* subset off kif: AD tarball, Kopia (calculon2), Documents/`/home`, Docker volumes — not full `/archive`, not `/media/projects`. USB/restic-copy rotation. Do this before #54. [#55](https://github.com/misnow1/home-network-v3/issues/55) |
| 23+ | `/media/projects` offline archive | Slice 23, 2×8TB dock | Finished trees archived to restic+LUKS on rotated CMR disks; active projects stay on RAID6. Helper scripts + runbook via Ansible; **not** the nightly timer. Dual-disk verify before unlink from `/media`. [#54](https://github.com/misnow1/home-network-v3/issues/54) |
